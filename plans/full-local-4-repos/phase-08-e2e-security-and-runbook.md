...
- The current working tree has no secret finding. The reachable-history audit currently reports historical API-key encryption material, so the rotation-evidence gate remains open until the affected key is rotated or the repository history is explicitly handled.
