...
{
    public class CallbackStepObserverWorker : BackgroundService
    {
        ...
        public CallbackStepObserverWorker([FromKeyedServices(QueueConstants.QUEUE_STEP_UPDATE)] RabbitMQ.Client.IModel stepChannel,
        ...
        protected async override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                if (change.Deleted || change.Document is null || !change.Document.WorkflowState.Equals(EWorkflowDataStatus.Reseting))
                    continue;
                ...
                try
                {
                    ...
                    foreach (var currentStepId in currentStepIdList)
                    {
                        ...
                        if (targetStep.CallbackObserved || !targetStep.IsRunCallBack) continue;
                        ...
                        switch (targetStep.State)
                        {
                            case EStepDataStatus.Done:
                                _logger.LogWarning("Callback Step {0} done. Continue.", targetStep.Step.Name);
                                File.WriteAllText(_lastSeqTxtpath, change.Seq);
                                PublishUnlockMessage(workflow.Steps[targetIndex].Executor);
                                PublishUpdateCallbackStepObservedMessage(workflow.Id, targetStep.Step.StepId, true);
                                break;

                            case EStepDataStatus.Failed:
                                _logger.LogWarning("Callback Step {0} failed.", targetStep.Step.Name);
                                File.WriteAllText(_lastSeqTxtpath, change.Seq);
                                PublishUnlockMessage(workflow.Steps[targetIndex].Executor);
                                var msg = $"Workflow {workflow.WorkflowName} failed at step {targetStep.Step.Sequence}: {targetStep.Step.Name}.";
                                PublishUpdateCallbackStepObservedMessage(workflow.Id, targetStep.Step.StepId, true, msg);
                                break;
        ...
        private void PublishUpdateCallbackStepObservedMessage(string docId, string stepId, bool observed, string? message = null)
        {
            var msg = new UpdateCallbackStepObservedMsg(docId, stepId, observed, message);

            var body = Encoding.UTF8.GetBytes(JsonConvert.SerializeObject(msg));

            var props = _stepChannel.CreateBasicProperties();
            props.Type = nameof(UpdateCallbackStepObservedMsg);
            _stepChannel.BasicPublish(
                exchange: QueueConstants.EXCHANGE_NAME,
                routingKey: QueueConstants.QUEUE_STEP_UPDATE_ROUTING_KEY,
                mandatory: true,
