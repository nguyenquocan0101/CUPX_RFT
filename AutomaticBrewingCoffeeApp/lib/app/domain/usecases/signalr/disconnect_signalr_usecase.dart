import 'package:abc_androidapp/app/core/exception/failure.dart';
import 'package:abc_androidapp/app/domain/repositories/signalr_repository.dart';
import 'package:fpdart/fpdart.dart';

class DisconnectSignalRUseCase {
  final SignalRRepository signalRRepository;

  DisconnectSignalRUseCase({required this.signalRRepository});

  Future<Either<Failure, void>> execute() async {
    try {
      await signalRRepository.disconnect();
      return const Right(null);
    } catch (e) {
      return Left(ServerFailure('Failed to disconnect from SignalR hub: ${e.toString()}'));
    }
  }
}